...
<body class="skin-yellow sidebar-mini" onload="load()">
	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
		<label class="control-label">我的生日:</label>
		<div class="controls">
		...
		<div class="control-group">
			 <label class="control-label">选择区域：</label>  
				<select  id="select" name="province" style="width:100px;" onchange="provinces(this)">
				    <c:forEach items="${ provinces}" var="province" >
					 ...
					 <label >：</label>  
						    <select id="city" name="city" style="width:100px;" name="city">
						          <option id="first">
						    </select>
	   </div> 
		<div class="control-group">
...
window.onload=function(){
		...
		var search="";
		$.post(          "${ctx}/sys/user/ajax",
				{"effective":word},
				function(data){
					if(data.length>0){
			    		 for(var i=0;i<data.length;i++){
						    	 search+="<option value='"+data[i].cityId+"'>"+data[i].cityName+"</option>"; 
						    }
