...
public class SystemService extends BaseService implements InitializingBean {
	...
	public Page<User> findUser(Page<User> page, User user) {
		...
		for (User userItem : list) {
			userItem.setCity(userDao.getCityNameById(userItem.getCity()));
			if(userItem.getBirthday()!=null&&!userItem.getBirthday().equals("")) {
				int length = userItem.getBirthday().toString().length()-4;
				String oldStringDate = userItem.getBirthday().toString().substring(length);
				int oldIntDate = Integer.parseInt(oldStringDate);
				Calendar newDate=Calendar.getInstance();
				int age = newDate.get(Calendar.YEAR);
				age=age-oldIntDate;
				userItem.setAge(age);
			}
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public List<City> getCityByProvince(String effective) {
		
		return userDao.getCityByProvince(effective);
	}
